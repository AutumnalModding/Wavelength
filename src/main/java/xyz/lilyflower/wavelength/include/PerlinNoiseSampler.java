...
public final class PerlinNoiseSampler {
	...
	private double sample(int sectionX, int sectionY, int sectionZ, double localX, double localY, double localZ, double fadeLocalX) {
		...
		double r = SolarisExtensions.perlinFade(localX);
		double s = SolarisExtensions.perlinFade(fadeLocalX);
		double t = SolarisExtensions.perlinFade(localZ);
		return SolarisExtensions.lerp3(r, s, t, d, e, f, g, h, o, p, q);
	}
	...
	private double sampleDerivative(int sectionX, int sectionY, int sectionZ, double localX, double localY, double localZ, double[] ds) {
		...
		double z = SolarisExtensions.perlinFade(localX);
		double aa = SolarisExtensions.perlinFade(localY);
		double ab = SolarisExtensions.perlinFade(localZ);
		double ac = SolarisExtensions.lerp3(z, aa, ab, is[0], js[0], ks[0], ls[0], ms[0], ns[0], os[0], ps[0]);
		double ad = SolarisExtensions.lerp3(z, aa, ab, is[1], js[1], ks[1], ls[1], ms[1], ns[1], os[1], ps[1]);
		double ae = SolarisExtensions.lerp3(z, aa, ab, is[2], js[2], ks[2], ls[2], ms[2], ns[2], os[2], ps[2]);
		double af = SolarisExtensions.lerp2(aa, ab, e - d, g - f, w - h, y - x);
		double ag = SolarisExtensions.lerp2(ab, z, f - d, x - h, g - e, y - w);
		double ah = SolarisExtensions.lerp2(z, aa, h - d, w - e, x - f, y - g);
		double ai = SolarisExtensions.perlinFadeDerivative(localX);
		double aj = SolarisExtensions.perlinFadeDerivative(localY);
		double ak = SolarisExtensions.perlinFadeDerivative(localZ);
		double al = ac + ai * af;
		double am = ad + aj * ag;
		double an = ae + ak * ah;
		ds[0] += al;
		ds[1] += am;
		ds[2] += an;
		return SolarisExtensions.lerp3(z, aa, ab, d, e, f, g, h, w, x, y);
	}
}